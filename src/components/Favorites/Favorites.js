            ...
            <div className="favorites">
                ...
                <ul className="favorites__list">
                    {this.state.favorits.map((item) => {
                        return <li key={item.id}><button className="remove" onClick={() => this.deleteBut(item.id)}><i class="fa-solid fa-trash"></i></button>  {item.title} ({item.year})</li>;
                    })}

                </ul>
                <button type="button" className={this.state.showLink ? 'favorites__save' : 'favorites__save-none'}
                    onClick={() => this.saveFavList()} disabled={!this.state.title}>Save List</button>
                <div className={this.state.showLink ? 'favorites-link' : ''}>
                    <Link to={`/list/${this.state.listId}`}>Filmlərin siyahısına keçid</Link>
                </div>
            </div>
